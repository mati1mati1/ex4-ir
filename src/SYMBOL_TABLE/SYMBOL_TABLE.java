...
//		PrintMe();
	...
	public void beginScope(String name,ScopeType type)
	{
		...
		enter(
				"SCOPE-BOUNDARY",
				new TYPE_FOR_SCOPE_BOUNDARIES(name,type));
...
//	}
	...
	public static SYMBOL_TABLE getInstance()
	{
		if (instance == null)
		{
			...
			/***************************************/
			/* [3] Enter library functions PrintInt & PrintString */
			/***************************************/
	...
	public TYPE findInCurrentClassScope(String name) {
		SYMBOL_TABLE_ENTRY e;

		for (e = table[hash(name)]; !(e.type instanceof TYPE_CLASS); e = e.next)
		{
			if (name.equals(e.name))
			{
				return e.type;
			...
			if (e.type instanceof TYPE_FOR_SCOPE_BOUNDARIES) {
				if (((TYPE_FOR_SCOPE_BOUNDARIES)e.type).type.equals(ScopeType.Global_Scope)){
					return null;
				}
			}
		}
	...
	public ScopeType getTypeOfScope() {
		SYMBOL_TABLE_ENTRY e = top;

		while (e != null && !(e.type instanceof TYPE_FOR_SCOPE_BOUNDARIES)) {
			e = e.prevtop;
		}
		return ((TYPE_FOR_SCOPE_BOUNDARIES)e.type).type;

	}
	public boolean isCurrentScopeIsClass(){
		SYMBOL_TABLE_ENTRY e = top;
	...
	public TYPE GetCurrentClassParent(){
		TYPE_CLASS currentClass = instance.getCurrentClass();
		if (currentClass == null){
			return null;
		}
		return currentClass.father;
	}
	...
	public TYPE extendedFind(String name) {
		TYPE this_scope = findInCurrentScope(name);
		if (this_scope != null) {
			return this_scope;
		}
		if(instance.isCurrentScopeIsClass()) { // in a class
			TYPE_CLASS tc = getCurrentClass();
			TYPE classVar = tc.checkIfClassMemberExistsInAncestors(name);
			if (classVar != null) { // exists in ancestors
				if (classVar instanceof TYPE_CLASS_VAR_DEC) { // is var in ancestors
					return ((TYPE_CLASS_VAR_DEC)classVar).t;
				} else { // is func in ancestors
					return classVar;
				}
			}  else { //does not exist in ancestors
				classVar = instance.findInCurrentClassScope(name); // check in current scope
				if (classVar != null) {
					return classVar;
				} else {
